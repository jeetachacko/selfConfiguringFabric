...
from config import (
    #REBUILD_LIMIT,
    #TX_DURATION,
    max_message_count, preferred_max_bytes, batch_timeout
)
import math
import numpy as np
...
class Fabric:
    ...
    def update_env_config(self, agent_conf, episode_step, fixed_config=True):
        ...
        rc = subprocess.call(["./scripts/k8s-updateconfig.sh", str(agent_conf[0]), str(agent_conf[1]), str(agent_conf[2])])
        #rc = subprocess.call("./scripts/k8s-execute-caliper.sh")
        ...
        try:
            ...
            self.current_state = (0, 0, 0, 0, 0)  # signal an error
