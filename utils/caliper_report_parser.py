...
def parse_caliper_log(keywords):
    ...
    try:
        ...
        wandb.log({'success': float(values[2].strip())})
        wandb.log({'fail': float(values[3].strip())})
        wandb.log({'send_rate': float(values[4].strip())})
        wandb.log({'avg_latency': float(values[7].strip())})
        wandb.log({'throughput': float(values[8].strip())})
        wandb.log({'successthroughput': successthroughput})
        wandb.log({'successrate': successrate})
        wandb.log({'relative_successthroughput': relative_successthroughput})
